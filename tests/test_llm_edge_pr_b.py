...
class Registry:
    ...
    def current(self, candidate_id):
        if candidate_id != self.state["candidate_id"]:
            return None
...
def _eval(checkpoint_n):
    return {
        "raw_n": checkpoint_n,
        "effective_n": checkpoint_n,
        "model": {"brier": 0.10, "logloss": 0.20, "raw_n": checkpoint_n, "effective_n": checkpoint_n},
        "baseline": {"brier": 0.15, "logloss": 0.30, "raw_n": checkpoint_n, "effective_n": checkpoint_n},
...
def test_pass_checkpoint_validates_then_promotes_exactly_once(monkeypatch):
    ...
    monkeypatch.setattr(evaluation, "_evaluate_sample", lambda rows, candidate, checkpoint_n: _eval(checkpoint_n))
    monkeypatch.setattr(evaluation, "_fdr_for_candidate", lambda *args, **kwargs: (0.001, "f" * 64))
...
def test_final_checkpoint_failure_never_promotes(monkeypatch):
    ...
    monkeypatch.setattr(evaluation, "_evaluate_sample", lambda rows, candidate, checkpoint_n: _eval(checkpoint_n))
    monkeypatch.setattr(evaluation, "_fdr_for_candidate", lambda *args, **kwargs: (0.50, "f" * 64))
    result = evaluation.evaluate_and_promote(engine, now=2000.0)
...
def test_checkpoint_and_promotion_storage_are_immutable():
    runtime = Runtime()
    evaluation.initialize_evaluation_storage(runtime)
    runtime._conn.execute("""INSERT INTO llm_edge_candidate_checkpoints(
          ...
          decision,evaluated_ts,contract_version
            ...
            "c", 24, "e", "DIRECTION", 60, "f", "s", 24, 24,
            "{}", "{}", "{}", 0.1, 0.01, 0.01,
            evaluation.LOOK_ADJUSTED_Q_MAX, "PASS", 1.0,
            evaluation.CONTRACT_VERSION,
        ))
    runtime._conn.commit()
    with pytest.raises(sqlite3.IntegrityError):
        runtime._conn.execute(
            "UPDATE llm_edge_candidate_checkpoints SET decision='FAIL' WHERE candidate_id='c'")
