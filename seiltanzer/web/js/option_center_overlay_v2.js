...
// Existing traces/canvas stay authoritative. Overlay traces are added only when
// missing after a structural Plotly.react; live updates use restyle in place.
...
const REJECTED_COLOR = 'rgba(138,135,125,0.9)';
...
export function optionCenterModel(cone, liveR = null) {
  ...
  return {
    ...
    accepted: source === 'bl_forward_shrunk' && rejectedGapR == null,
    rejectedGapR,
    source,
  };
}
...
function coneTracePayload(el, state) {
  ...
  const raw = {
    ...
    marker: {
      size: 7,
      color: model.accepted ? RAW_COLOR : REJECTED_COLOR,
      symbol: 'diamond-open',
...
async function addMissingConeTraces(el, payload) {
  const P = window.Plotly;
  const missing = [];
  if (traceIndex(el, ROBUST_META) < 0) missing.push(payload.robust);
  if (traceIndex(el, RAW_META) < 0) missing.push(payload.raw);
  if (!missing.length) return false;
  const camera = cloneCamera(el);
  await P.addTraces(el, missing);
  if (camera) await P.relayout(el, { 'scene.camera': camera });
  return true;
}

async function restyleConeTraces(el, payload) {
  const P = window.Plotly;
  const robustIndex = traceIndex(el, ROBUST_META);
  const rawIndex = traceIndex(el, RAW_META);
  if (robustIndex < 0 || rawIndex < 0) return false;
  await P.restyle(el, {
    x: [payload.robust.x], y: [payload.robust.y], z: [payload.robust.z],
    name: payload.robust.name,
    hovertemplate: payload.robust.hovertemplate,
  }, [robustIndex]);
  await P.restyle(el, {
    x: [payload.raw.x], y: [payload.raw.y], z: [payload.raw.z],
    name: payload.raw.name,
    hovertemplate: payload.raw.hovertemplate,
    'marker.color': payload.raw.marker.color,
  }, [rawIndex]);
  return true;
}
...
async function syncConeOverlays(state) {
  ...
  const payload = coneTracePayload(el, state);
  if (!payload) {
    await deleteConeOverlays(el);
    return;
  }
  // Plotly.react from cone_core removes appended traces on structural changes.
  // Add only those missing traces; ordinary price ticks update them in place.
  const added = await addMissingConeTraces(el, payload);
  if (!added) await restyleConeTraces(el, payload);
}
...
function drawFanOverlay(state) {
  ...
  ctx.fillText(`ROBUST FWD ${model.robustForwardR >= 0 ? '+' : ''}${model.robustForwardR.toFixed(2)}R`,
    width - padR - 8, clamp(Y(model.robustForwardR) - 6, padT + 9, H - padB - 5));
  ctx.fillStyle = model.accepted ? RAW_COLOR : 'rgba(138,135,125,0.95)';
  const rawLabel = model.accepted ? 'RND MEAN H' : 'RND MEAN H · REJECTED';
  ctx.fillText(`${rawLabel} ${model.rawMeanR >= 0 ? '+' : ''}${model.rawMeanR.toFixed(2)}R`,
    width - padR - 8, clamp(rawY + 13, padT + 10, H - padB - 4));
}

const state = {
  cone: null, liveR: null, curR: null, liveImpulse: 0, trail: [],
  coneTimer: null, syncRunning: false, syncAgain: false,
};

function scheduleConeSync(delay = 45) {
  if (!browser) return;
  if (state.coneTimer) clearTimeout(state.coneTimer);
  state.coneTimer = setTimeout(async () => {
    state.coneTimer = null;
    if (state.syncRunning) { state.syncAgain = true; return; }
    state.syncRunning = true;
    try {
      await syncConeOverlays(state);
    } catch {
