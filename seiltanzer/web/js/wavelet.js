...
function onMarketTick(tick) {
  ...
  if (Number(tick.impulse || 0) > (mobile ? .018 : .012)) {
    liveBursts.push({ born: performance.now(), impulse: Number(tick.impulse || 0), direction: Number(tick.direction || 0) });
    const cap = mobile ? 10 : 24;
    if (liveBursts.length > cap) liveBursts = liveBursts.slice(-cap);
  }
...
function ensureCanvas() {
  ...
  const width=mobile?Math.max(280,Math.floor(rect.width||340)):Math.max(540,Math.floor(rect.width||850));
  ...
  return {cv,dpr,width,height,mobile};
}
...
function drawSpectrogram({cv,dpr,width,height,mobile},now){
  ...
  const margin=mobile?{left:46,right:12,top:18,bottom:32}:{left:64,right:22,top:22,bottom:38};const plotW=width-margin.left-margin.right,plotH=height-margin.top-margin.bottom;
  ...
  const contourStep=mobile?2:1;
  for(let r=0;r<rows;r++)for(let c=0;c<cols;c+=contourStep){const v=Number(spec[r]?.[c]||0);if(v<(mobile?.78:.72))continue;const left=Number(spec[r]?.[Math.max(0,c-1)]||0),right=Number(spec[r]?.[Math.min(cols-1,c+1)]||0),up=Number(spec[Math.min(rows-1,r+1)]?.[c]||0),down=Number(spec[Math.max(0,r-1)]?.[c]||0);if(Math.min(left,right,up,down)<.62)ctx.strokeRect(margin.left+c*cellW,margin.top+plotH-(r+1)*cellH,Math.ceil(cellW)+1,Math.ceil(cellH)+1);}
  ctx.font=`${mobile?8:9}px IBM Plex Mono,monospace`;ctx.fillStyle='rgba(206,220,230,.72)';ctx.strokeStyle='rgba(191,210,223,.13)';
  grid.forEach((p,r)=>{const y=margin.top+plotH-(r+.5)*cellH;ctx.beginPath();ctx.moveTo(margin.left,y);ctx.lineTo(margin.left+plotW,y);ctx.stroke();ctx.textAlign='right';ctx.textBaseline='middle';ctx.fillText(`${p}h`,margin.left-5,y);});
  const labelCount=mobile?3:5;for(let i=0;i<labelCount;i++){const idx=Math.round(i*(cols-1)/Math.max(1,labelCount-1));const x=margin.left+(idx+.5)*cellW;ctx.textAlign='center';ctx.textBaseline='top';ctx.fillText(fmtTime(timestamps[idx],!mobile),x,margin.top+plotH+7);}
  const periodIndex=new Map(grid.map((p,i)=>[Number(p),i]));
  const drawRidge=(field,color,widthLine,dash=[])=>{ctx.save();ctx.setLineDash(dash);ctx.strokeStyle='rgba(3,9,14,.78)';ctx.lineWidth=widthLine+4;ctx.beginPath();let started=false;ridge.forEach((pt,c)=>{const row=periodIndex.get(Number(pt[field]));if(row==null)return;const x=margin.left+(c+.5)*cellW,y=margin.top+plotH-(row+.5)*cellH;if(!started){ctx.moveTo(x,y);started=true;}else ctx.lineTo(x,y);});ctx.stroke();ctx.strokeStyle=color;ctx.shadowColor=color;ctx.shadowBlur=mobile?5:8;ctx.lineWidth=widthLine;ctx.stroke();ctx.restore();};
  drawRidge('period_hours','#55e4f1',mobile?2:2.4);drawRidge('secondary_period_hours','rgba(241,187,74,.72)',1.2,[4,4]);
  liveBursts.forEach((b)=>{const age=(now-b.born)/2400,alpha=(1-age)*b.impulse;const x=margin.left+plotW-age*plotW*.055;const grad=ctx.createLinearGradient(0,margin.top,0,margin.top+plotH);grad.addColorStop(0,`rgba(255,93,116,${alpha*.15})`);grad.addColorStop(.72,`rgba(84,222,224,${alpha*.22})`);grad.addColorStop(1,`rgba(84,222,224,${alpha*.75})`);ctx.strokeStyle=grad;ctx.lineWidth=1+4*alpha;ctx.shadowColor=b.direction>=0?'#55e4f1':'#ff5d74';ctx.shadowBlur=(mobile?7:12)*alpha;ctx.beginPath();ctx.moveTo(x,margin.top);ctx.lineTo(x,margin.top+plotH);ctx.stroke();ctx.shadowBlur=0;});
  const dom=Number(waveletData.summary?.dominant_period_hours||0),di=grid.findIndex(p=>Number(p)===dom);if(di>=0){const y=margin.top+plotH-(di+.5)*cellH;ctx.fillStyle='#f3c36a';ctx.font=`bold ${mobile?8:9}px IBM Plex Mono,monospace`;ctx.textAlign='right';ctx.fillText(`NOW ${dom.toFixed(1)}h`,margin.left+plotW-5,y-5);}
...
function renderSurface(force=false){
  ...
  const ts=keep(ts0),spec=spec0.map((row)=>keep(row)),ridge=keep(ridge0);const latest=Number(ts0.at(-1)||0);const x=ts.map(t=>(Number(t)-latest)/3600);
  const traces=[{type:'surface',x,y:periods,z:spec,colorscale:[[0,'#07101b'],[.2,'#123a5b'],[.42,'#1e8798'],[.64,'#36c9a7'],[.82,'#e2b348'],[1,'#ef5268']],cmin:0,cmax:1,showscale:false,opacity:.94,hovertemplate:mobile?undefined:'t=%{x:.2f}h<br>period=%{y:.1f}h<br>power=%{z:.2f}<extra></extra>',hoverinfo:mobile?'skip':undefined,contours:{z:{show:!mobile,color:'rgba(232,240,245,.18)',project:{z:true}}},lighting:{ambient:.42,diffuse:.72,roughness:.80,specular:mobile?.12:.25},lightposition:{x:70,y:-80,z:120}},
    {type:'scatter3d',mode:'lines',name:'DOMINANT RIDGE',x:ridge.map(p=>(Number(p.ts)-latest)/3600),y:ridge.map(p=>Number(p.period_hours)),z:ridge.map(p=>1.035+Number(p.power||0)*.045),line:{color:'#59eff2',width:mobile?5:7},hoverinfo:'skip',showlegend:false},
...
function updateSurfaceProbe(){
  if(currentMode!=='SURFACE'||!window.Plotly||!containerEl)return;const plot=containerEl.querySelector('[data-renderer="wavelet-surface"]');if(!plot?.data)return;const idx=plot.data.findIndex(t=>t.name==='LIVE TICK PROBE');if(idx<0)return;const imp=clamp(Number(liveTick?.impulse||0),0,1),dom=Number(waveletData?.summary?.dominant_period_hours||1);window.Plotly.restyle(plot,{y:[[dom]],z:[[1.06+.14*imp]],'marker.size':[[isAnalyticsMobile()?6+7*imp:7+11*imp]]},[idx]);
}
