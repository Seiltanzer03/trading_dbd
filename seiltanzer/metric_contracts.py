...
CONTRACTS: tuple[MetricContract, ...] = (
    ...
    _m("option.barrier_ev_r", "option first-touch probabilities + final take R",
       ...
       "KEEP + IMPROVE"),
    _m("option.first_touch_median", "resolved take and stop path times",
       "median(time | either barrier touched)", "minutes or years",
       "same as first-touch paths", "null when no path resolves",
       "option_distribution", "Typical time to any barrier conditional on resolution.",
       "Time context only until take/stop conditional medians are split.",
       "Same path family as hazards and touch probabilities.",
       "current field mixes take and stop; can be mistaken for take median",
       "BROKEN / FIX"),
