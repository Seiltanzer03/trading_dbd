...
STATUS_RU = {
    ...
    "EARLY": "МАЛО ДАННЫХ",
    "EARLY_ADVANTAGE": "РАННИЙ ПЕРЕВЕС",
    "EARLY_DISADVANTAGE": "РАННИЙ АНТИ-СИГНАЛ",
    "EARLY_MIXED": "СМЕШАННЫЙ РЕЗУЛЬТАТ",
    "EARLY_UNDECIDED": "ПОКА НЕТ ВЕРДИКТА",
...
def build_ml_research_broadcast(app: FastAPI, *, now: float | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "researcher_summary": {
            ...
            "early_evaluated": _integer(researcher.get("early_evaluated")),
            "early_advantage": _integer(researcher.get("early_advantage")),
            "early_disadvantage": _integer(researcher.get("early_disadvantage")),
            "early_mixed": _integer(researcher.get("early_mixed")),
            "early_undecided": _integer(researcher.get("early_undecided")),
