...
Обязательные разделы: ДЕЙСТВИЕ, ОБЩАЯ ГЕОМЕТРИЯ СЦЕНАРИЕВ, РАСЧЁТ ПОЛИТИК,
ПОЧЕМУ ВЫБРАНО, ПОДТВЕРЖДЕНИЯ И ПРОТИВОРЕЧИЯ, РАЗЛОЖЕНИЕ ИЗМЕНЕНИЯ,
ПОСЛЕ ИСПОЛНЕНИЯ, ГРАНИЦА ОТМЕНЫ, СЛЕДУЮЩИЙ ПЕРЕСЧЁТ, КАЧЕСТВО ДАННЫХ.
