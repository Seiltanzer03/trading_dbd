...
<div class="card wide"><div class="k">КАК ЧАСТО ПЕРЕВЕСЫ ВСТРЕЧАЮТСЯ И ВЛИЯЮТ НА РЕШЕНИЕ</div><div style="overflow:auto"><table><thead><tr><th>Период</th><th>Разборов</th><th>Совпал edge-контекст</th><th>Раннее преимущество</th><th>Гипотеза хуже базовой</th><th>Изменил первичный выбор</th></tr></thead><tbody id="frequency"></tbody></table></div><div class="note" id="frequency-note">Disadvantage означает, что гипотеза не обошла базовый прогноз; это не автоматический обратный сигнал. Счётчик начинается после активации.</div></div>
...
async function load(){try{
 const [s,e,p,o,f]=await Promise.all([json('/api/research/g1/management/status'),json('/api/research/g1/management/edge'),json('/api/research/g1/management/policies'),json('/api/research/g1/management/observations?limit=25'),json('/api/research/g1/management/edge-frequency').catch(()=>({windows:{}}))]);
 ...
 const labels={'24h':'24 часа','7d':'7 дней','30d':'30 дней','all':'Всё время'};frequency.innerHTML=['24h','7d','30d','all'].map(k=>{const x=f.windows?.[k]||{};return `<tr><td>${labels[k]}</td><td>${x.eligible_reviews||0}</td><td>${pct(x.any_edge_match_rate)}</td><td>${pct(x.early_advantage_rate)}</td><td>${pct(x.early_disadvantage_rate)}</td><td>${pct(x.raw_choice_changed_rate)}</td></tr>`}).join('');frequency_note.textContent=f.contract_version?`Только будущие уникальные разборы · активация ${f.activation_ts?new Date(f.activation_ts*1000).toLocaleString('ru-RU'):'—'} · historical backfill OFF`:'Счётчик частоты временно недоступен; остальные данные страницы актуальны.';
