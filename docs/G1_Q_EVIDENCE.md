...
- `q-native-expiry-cadence-v1` — capture policy;
- existing `option-q-contract-f32-v1` and `act365-calendar-f32-v1` remain authoritative.

## Evidence flow

`background collector -> option source -> frozen T0 terminal Q -> expiry -> F.3.2a truth -> G.1A admission -> G.1B metrics`

Failed Q attempts are never converted into synthetic observations. The ledger
records deterministic blockers such as missing provider/chain, stale source,
invalid expiry, insufficient density support, invalid CDF, proxy mapping errors,
non-direct/stale target prices, and persistence failures.

## Counts are intentionally separate

- capture attempts: real background attempts;
...
snapshot and a fresh direct target-market price at T0. Cached/stale option data
or proxy target prices may remain useful elsewhere in the terminal but cannot
be counted as successful G.1B.1 evidence.

Fixed 15m/30m/60m/2h/4h/8h/24h forecasts are not relabeled as Q. Only the
option-native expiry cohort can carry `risk_neutral_Q_terminal`.
